...
import com.github.generator.common.config.Global;
import com.github.generator.common.mapper.JaxbMapper;
import com.github.generator.common.utils.DateUtils;
import com.github.generator.common.utils.FileUtils;
import com.github.generator.common.utils.FreeMarkers;
...
public class GenUtils {
    ...
    public static void initColumnField(GenTable genTable) {
        for (GenTableColumn column : genTable.getColumnList()) {
            ...
            } else if (StringUtils.startsWithIgnoreCase(column.getJdbcType(), "BIGDECIMAL")) {
            ...
            if (StringUtils.equalsIgnoreCase(column.getName(), "create_time")
                    || StringUtils.equalsIgnoreCase(column.getName(), "update_time")
                    || StringUtils.equalsIgnoreCase(column.getName(), "valid") ) {
                column.setIsQuery("0");
            } else {
                column.setIsQuery("1");
            }
    ...
    public static Map<String, Object> getDataModel(GenScheme genScheme) {
        ...
        //切换模块
        model.put("changeModule", genScheme.getChangeModule());

        //是否继承BaseEntity
        model.put("extendBase", genScheme.getGenTable().getExtendBase());

        //是否自动生成主键
        //model.put("autoIncrease", genScheme.getGenTable().get)
    ...
    public static String generateToFile(GenTemplate tpl, Map<String, Object> model, boolean isReplaceFile) {
        ...
        Object changeModule = model.get("changeModule");
        //切换模块
        if (changeModule != null && StringUtils.isNotBlank((changeModule.toString()) )) {
            path = path.substring(0, path.lastIndexOf(File.separator)+1);
            path += changeModule;
        }
        String fileName = StringUtils.replaceEach(FreeMarkers.renderString(path + File.separator + tpl.getFilePath() + "/", model),
